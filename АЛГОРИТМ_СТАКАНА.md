...
graph TD
    ...
    O --> R["book_l3 (MBO snapshots)"]
    P --> Q["book_l2 (MBP snapshots)"]
    M --> S["book_l1 (BBO time series)"]
```
...
`scripts/build_orderbook.py` создаёт по одному каталогу для каждого тикера (`data/orderbook/<ticker>/`) и кладёт туда одинаковый набор файлов:

| Файл | Содержание |
|------|------------|
| `book_l3.parquet` / `book_l3.csv` | Полные MBO-снимки после каждого `EndOfEvent` (позиционный стакан со всеми ордерами и `position_no`). |
| `book_l2.parquet` / `book_l2.csv` | Агрегированные ценовые уровни (MBP) для тех же событий; каждая строка = уровень с суммарным объёмом и числом ордеров. |
| `book_l1.parquet` / `book_l1.csv` | Временной ряд BBO: `bid_price/bid_size/ask_price/ask_size` после каждого события. |
