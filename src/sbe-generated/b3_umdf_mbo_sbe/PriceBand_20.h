...
#ifndef _B3_UMDF_MBO_SBE_PRICEBAND_20_CXX_H_
#define _B3_UMDF_MBO_SBE_PRICEBAND_20_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(20);
    ...
    PriceBand_20(char *buffer, const std::uint64_t bufferLength) :
        PriceBand_20(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    PriceBand_20(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        PriceBand_20(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    PriceBand_20 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PriceBand_20 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        PriceBand_20 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    PriceBand_20 &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    PriceBand_20 &priceBandType(const PriceBandType::Value value) SBE_NOEXCEPT
    {
    ...
    PriceBand_20 &priceLimitType(const PriceLimitType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
    ...
    PriceBand_20 &priceBandMidpointPriceType(const PriceBandMidpointPriceType::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
...
private:
    PriceOptional m_tradingReferencePrice;

public:
    SBE_NODISCARD PriceOptional &tradingReferencePrice()
    {
...
public:
    ...
    PriceBand_20 &rptSeq(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const PriceBand_20 &_writer)
{
    PriceBand_20 writer(
        _writer.m_buffer,
        _writer.m_offset,
