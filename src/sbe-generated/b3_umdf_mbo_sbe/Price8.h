...
#ifndef _B3_UMDF_MBO_SBE_PRICE8_CXX_H_
#define _B3_UMDF_MBO_SBE_PRICE8_CXX_H_
...
public:
    ...
    Price8(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Price8(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Price8(
        char *buffer,
        const std::uint64_t bufferLength) :
        Price8(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Price8 &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Price8 &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
