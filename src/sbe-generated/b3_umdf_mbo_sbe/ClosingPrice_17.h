...
#ifndef _B3_UMDF_MBO_SBE_CLOSINGPRICE_17_CXX_H_
#define _B3_UMDF_MBO_SBE_CLOSINGPRICE_17_CXX_H_
...
public:
    ...
    ClosingPrice_17(char *buffer, const std::uint64_t bufferLength) :
        ClosingPrice_17(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ClosingPrice_17(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ClosingPrice_17(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ClosingPrice_17 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ClosingPrice_17 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        ClosingPrice_17 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    ClosingPrice_17 &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    {
        return MDEntryType::Value::CLOSING_PRICE;
    }
    ...
    {
        return static_cast<char>(MDEntryType::Value::CLOSING_PRICE);
    }
    ...
    ClosingPrice_17 &openCloseSettlFlag(const OpenCloseSettlFlag::Value value) SBE_NOEXCEPT
    {
...
public:
    SBE_NODISCARD Price8 &mDEntryPx()
    {
        m_mDEntryPx.wrap(m_buffer, m_offset + 12, m_actingVersion, m_bufferLength);
        return m_mDEntryPx;
    }

    SBE_NODISCARD static const char *lastTradeDateMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
    ...
    static SBE_CONSTEXPR std::uint16_t lastTradeDateId() SBE_NOEXCEPT
    {
        return 9325;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t lastTradeDateSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool lastTradeDateInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t lastTradeDateEncodingOffset() SBE_NOEXCEPT
    {
        return 20;
    }

    static SBE_CONSTEXPR std::uint16_t lastTradeDateNullValue() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(0);
    }

    static SBE_CONSTEXPR std::uint16_t lastTradeDateMinValue() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(0);
    }

    static SBE_CONSTEXPR std::uint16_t lastTradeDateMaxValue() SBE_NOEXCEPT
    {
        return static_cast<std::uint16_t>(65534);
    }

    static SBE_CONSTEXPR std::size_t lastTradeDateEncodingLength() SBE_NOEXCEPT
    {
        return 2;
    }

    SBE_NODISCARD std::uint16_t lastTradeDate() const SBE_NOEXCEPT
    {
        std::uint16_t val;
    ...
    ClosingPrice_17 &lastTradeDate(const std::uint16_t value) SBE_NOEXCEPT
    {
    ...
    ClosingPrice_17 &tradeDate(const std::uint16_t value) SBE_NOEXCEPT
    {
        std::uint16_t val = SBE_LITTLE_ENDIAN_ENCODE_16(value);
...
public:
    ...
    {
        m_mDEntryTimestamp.wrap(m_buffer, m_offset + 24, m_actingVersion, m_bufferLength);
    ...
    ClosingPrice_17 &rptSeq(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ClosingPrice_17 &_writer)
{
    ClosingPrice_17 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ClosingPrice_17", )";
    ...
    builder << ", ";
    builder << R"("lastTradeDate": )";
    builder << +writer.lastTradeDate();
