...
#ifndef _B3_UMDF_MBO_SBE_PERCENTAGE9_CXX_H_
#define _B3_UMDF_MBO_SBE_PERCENTAGE9_CXX_H_
...
public:
    ...
    Percentage9(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Percentage9(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Percentage9(
        char *buffer,
        const std::uint64_t bufferLength) :
        Percentage9(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Percentage9 &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    {
        return static_cast<std::int8_t>(-9);
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Percentage9 &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
