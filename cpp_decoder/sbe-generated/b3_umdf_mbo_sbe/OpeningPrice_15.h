...
#ifndef _B3_UMDF_MBO_SBE_OPENINGPRICE_15_CXX_H_
#define _B3_UMDF_MBO_SBE_OPENINGPRICE_15_CXX_H_
...
public:
    ...
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(15);
    ...
    OpeningPrice_15(char *buffer, const std::uint64_t bufferLength) :
        OpeningPrice_15(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OpeningPrice_15(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OpeningPrice_15(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OpeningPrice_15 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OpeningPrice_15 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OpeningPrice_15 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    OpeningPrice_15 &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    OpeningPrice_15 &mDUpdateAction(const MDUpdateAction::Value value) SBE_NOEXCEPT
    {
    ...
    OpeningPrice_15 &openCloseSettlFlag(const OpenCloseSettlFlag::Value value) SBE_NOEXCEPT
    {
        std::uint8_t val = (value);
...
public:
    ...
    {
        m_netChgPrevDay.wrap(m_buffer, m_offset + 20, m_actingVersion, m_bufferLength);
    ...
    OpeningPrice_15 &tradeDate(const std::uint16_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    {
        m_mDEntryTimestamp.wrap(m_buffer, m_offset + 30, m_actingVersion, m_bufferLength);
    ...
    {
        std::uint32_t val;
        std::memcpy(&val, m_buffer + m_offset + 38, sizeof(std::uint32_t));
        return SBE_LITTLE_ENDIAN_ENCODE_32(val);
    }

    OpeningPrice_15 &rptSeq(const std::uint32_t value) SBE_NOEXCEPT
    {
        std::uint32_t val = SBE_LITTLE_ENDIAN_ENCODE_32(value);
        std::memcpy(m_buffer + m_offset + 38, &val, sizeof(std::uint32_t));
        return *this;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OpeningPrice_15 &_writer)
{
    OpeningPrice_15 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "OpeningPrice_15", )";
