...
#ifndef _B3_UMDF_MBO_SBE_PRICEOFFSET8OPTIONAL_CXX_H_
#define _B3_UMDF_MBO_SBE_PRICEOFFSET8OPTIONAL_CXX_H_
...
public:
    ...
    PriceOffset8Optional(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        PriceOffset8Optional(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    PriceOffset8Optional(
        char *buffer,
        const std::uint64_t bufferLength) :
        PriceOffset8Optional(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    PriceOffset8Optional &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    PriceOffset8Optional &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    {
        return static_cast<std::int8_t>(-8);
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, PriceOffset8Optional &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
