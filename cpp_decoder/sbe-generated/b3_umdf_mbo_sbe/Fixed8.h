...
#ifndef _B3_UMDF_MBO_SBE_FIXED8_CXX_H_
#define _B3_UMDF_MBO_SBE_FIXED8_CXX_H_
...
public:
    ...
    Fixed8(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Fixed8(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Fixed8(
        char *buffer,
        const std::uint64_t bufferLength) :
        Fixed8(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Fixed8 &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Fixed8 &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
