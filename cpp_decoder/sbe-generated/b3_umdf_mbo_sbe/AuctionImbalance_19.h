...
#ifndef _B3_UMDF_MBO_SBE_AUCTIONIMBALANCE_19_CXX_H_
#define _B3_UMDF_MBO_SBE_AUCTIONIMBALANCE_19_CXX_H_
...
public:
    ...
    AuctionImbalance_19(char *buffer, const std::uint64_t bufferLength) :
        AuctionImbalance_19(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    AuctionImbalance_19(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        AuctionImbalance_19(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    AuctionImbalance_19 &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    AuctionImbalance_19 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    AuctionImbalance_19 &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        AuctionImbalance_19 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    AuctionImbalance_19 &securityID(const std::uint64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    AuctionImbalance_19 &mDUpdateAction(const MDUpdateAction::Value value) SBE_NOEXCEPT
    {
    ...
    {
        return MDEntryType::Value::IMBALANCE;
    }
    ...
    {
        return static_cast<char>(MDEntryType::Value::IMBALANCE);
    }

    SBE_NODISCARD static const char *imbalanceConditionMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t imbalanceConditionId() SBE_NOEXCEPT
    {
        return 37277;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t imbalanceConditionSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool imbalanceConditionInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t imbalanceConditionEncodingOffset() SBE_NOEXCEPT
    {
        return 10;
    }

private:
    ImbalanceCondition m_imbalanceCondition;

public:
    SBE_NODISCARD ImbalanceCondition &imbalanceCondition()
    {
        m_imbalanceCondition.wrap(m_buffer, m_offset + 10, m_actingVersion, m_bufferLength);
        return m_imbalanceCondition;
    }

    static SBE_CONSTEXPR std::size_t imbalanceConditionEncodingLength() SBE_NOEXCEPT
    {
        return 2;
    }
    ...
    AuctionImbalance_19 &mDEntrySize(const std::int64_t value) SBE_NOEXCEPT
    {
...
public:
    ...
    {
        m_mDEntryTimestamp.wrap(m_buffer, m_offset + 20, m_actingVersion, m_bufferLength);
    ...
    AuctionImbalance_19 &rptSeq(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const AuctionImbalance_19 &_writer)
{
    AuctionImbalance_19 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "AuctionImbalance_19", )";
    ...
    builder << ", ";
    builder << R"("imbalanceCondition": )";
    builder << writer.imbalanceCondition();
