...
#ifndef _B3_UMDF_MBO_SBE_UTCTIMESTAMPSECONDS_CXX_H_
#define _B3_UMDF_MBO_SBE_UTCTIMESTAMPSECONDS_CXX_H_
...
public:
    ...
    UTCTimestampSeconds(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        UTCTimestampSeconds(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    UTCTimestampSeconds(
        char *buffer,
        const std::uint64_t bufferLength) :
        UTCTimestampSeconds(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    UTCTimestampSeconds &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    UTCTimestampSeconds &time(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, UTCTimestampSeconds &writer)
{
    builder << '{';
    builder << R"("time": )";
    builder << +writer.time();
